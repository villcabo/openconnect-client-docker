...
kill_existing_openconnect() {
    log info "Checking for existing OpenConnect processes ${ICON_LOADING}..."
    
    # Check if OpenConnect process is running in container
    if docker exec "${CONTAINER_NAME}" pgrep -x openconnect >/dev/null 2>&1; then
        log warning "Found existing OpenConnect process. Stopping it ${ICON_WARNING}..."
        
        # Kill OpenConnect process
        docker exec "${CONTAINER_NAME}" pkill -x openconnect 2>/dev/null
        ...
        # Verify it's stopped
        if docker exec "${CONTAINER_NAME}" pgrep -x openconnect >/dev/null 2>&1; then
            log warning "OpenConnect process still running. Force killing ${ICON_WARNING}..."
            docker exec "${CONTAINER_NAME}" pkill -9 openconnect 2>/dev/null
            sleep 1
        fi
        
        log success "Existing OpenConnect process stopped ${ICON_SUCCESS}."
    else
        log info "No existing OpenConnect process found ${ICON_SUCCESS}."
    fi
}

is_service_running() {
    log debug "Starting OpenConnect service validation..."
    
    # Check if OpenConnect process is running
    log debug "Checking if OpenConnect process is running in container..."
    if ! docker exec "${CONTAINER_NAME}" pgrep -x openconnect >/dev/null 2>&1; then
    ...
    log debug "Checking for VPN interface (tun0)..."
...
wait_for_vpn_startup() {
    ...
    log warning "VPN startup message not detected within 10 seconds, continuing anyway ${ICON_WARNING}..."
    return 1
}
...
start_vpn() {
    ...
    section "[2/4] OpenConnect Service Validation"
    log info "Checking OpenConnect service status ${ICON_LOADING}..."
    if is_service_running; then
        log success "OpenConnect service is active in the container ${ICON_SUCCESS}."
    else
        log warning "Container is running but OpenConnect service is not active ${ICON_WARNING}."
        log info "Trying to start OpenConnect service inside the container ${ICON_LOADING}..."
        if docker exec -d "${CONTAINER_NAME}" bash "${ENTRYPOINT_SCRIPT}"; then
            sleep 3
            wait_for_vpn_startup
            if is_service_running; then
                log success "OpenConnect service started successfully ${ICON_SUCCESS}."
            else
                log error "Could not start OpenConnect service ${ICON_ERROR}."
                exit 1
            fi
        else
    ...
    section "[3/4] Route Configuration"
